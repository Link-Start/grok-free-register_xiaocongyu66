...
class TurnstileAPIServer:
    # Minimal page used to host Turnstile. Loading the real target page often
    # hits CSP and never creates cf-turnstile-response; fulfill() avoids that.
    HTML_TEMPLATE = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    async def _solve_turnstile(self, task_id: str, url: str, sitekey: str, action: Optional[str] = None, cdata: Optional[str] = None):
        ...
        try:
            ...
            # Serve a minimal same-origin page so Turnstile can write the token
            # without the real site's CSP blocking challenges.cloudflare.com.
            url_with_slash = url if url.endswith("/") else (url + "/")
            attrs = f'data-sitekey="{sitekey}"'
            if action:
                attrs += f' data-action="{action}"'
            if cdata:
                attrs += f' data-cdata="{cdata}"'
            turnstile_div = (
                f'<div class="cf-turnstile" style="background:white;width:300px;height:70px;" {attrs}></div>'
            )
            ...
            # Also fulfill without trailing slash variants
            await page.route(
                url.rstrip("/"),
                lambda route: route.fulfill(
                    body=page_data,
            ...
            # Do not install broad route blocking here: it can override the
            # fulfill() handler for the synthetic page and break Turnstile JS.
            await page.goto(url_with_slash, wait_until="domcontentloaded", timeout=45000)

            try:
                await page.eval_on_selector(
                    "//div[@class='cf-turnstile']",
                    "el => { el.style.width = '300px'; el.style.height = '70px'; }",
                )
            except Exception:
                pass
            ...
            locator = page.locator('input[name="cf-turnstile-response"], textarea[name="cf-turnstile-response"]')
            max_attempts = 30
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    for i in range(max(count, 0)):
                        try:
                            value = await locator.nth(i).input_value(timeout=500)
                    ...
                    if attempt in (0, 5, 10, 15, 20, 25) and self.debug:
                        logger.debug(f"Browser {index}: Attempt {attempt + 1}/{max_attempts} - No valid token yet")

                    # periodic click to trigger interactive challenge
                    if attempt > 0 and attempt % 2 == 0:
                        try:
                            await page.locator(".cf-turnstile").first.click(timeout=1000)
                        except Exception:
                            try:
                    ...
                    await asyncio.sleep(min(0.6 + attempt * 0.05, 2.0))
                except Exception as e:
                    if self.debug:
