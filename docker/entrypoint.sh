...
# 新版 hybrid 默认 8 浏览器并发（可用 Secrets 覆盖）
export SOLVER_GATEWAY_WORKERS="${SOLVER_GATEWAY_WORKERS:-8}"
export SOLVER_GATEWAY_WORKERS_MAX="${SOLVER_GATEWAY_WORKERS_MAX:-8}"
export SOLVER_WORKER_CONCURRENCY="${SOLVER_WORKER_CONCURRENCY:-1}"
export TURNSTILE_SOLVER_THREADS="${TURNSTILE_SOLVER_THREADS:-8}"
...
grep -q '^TURNSTILE_SOLVER_THREADS=' /app/.env 2>/dev/null || echo "TURNSTILE_SOLVER_THREADS=${TURNSTILE_SOLVER_THREADS}" >> /app/.env
