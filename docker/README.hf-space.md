...
- 新版 hybrid：`SOLVER_GATEWAY_WORKERS=auto`（按 CPU/内存定浏览器数，请求进队列）
- 需要硬顶时只改 `SOLVER_GATEWAY_WORKERS_MAX`（默认 8）
