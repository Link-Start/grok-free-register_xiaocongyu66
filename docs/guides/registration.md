...
`代理.txt` 也可以直接写 `vmess`、`vless`、`trojan`、`ss`、`hy2`、`hysteria2`、`tuic`、`anytls` 分享链接。使用分享链接时需先启动本机 proxy-relay 服务，默认管理地址是 `http://127.0.0.1:18080`：
...
可选值支持 `legacy`、`sub2api`/`sub`、`cpa`，例如 `KEY_EXPORT_FORMATS=sub2api,cpa`。`sub2api` 文件写入 `keys/sub2api/`，CPA 文件写入 `keys/cpa/`。`keys/auth-sessions.jsonl` 会保留为自动认证恢复源。

如果邮箱服务接口偶发 Cloudflare 拦截，可以开启 CF-Ares 兜底；`cf-ares` 已随默认依赖安装：
...
成功结果写入 `keys/`；默认会有 `accounts.txt`、`grok.txt`、`auth-sessions.jsonl` 和 `sub2api/`。这些文件默认不提交到 Git。
