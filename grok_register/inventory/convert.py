...
def convert_accounts(
    ...
    *,
    ...
    Prefer Go inventory-worker convert (OAuth file transform + optional protocol SSO).
    Enroll (protocol / browser) is opt-in via allow_enroll=True — used by「仅转换待 OAuth」.
    """
    root = key_export_dir()
    ...
    use_go = engine not in {"python", "py"}
    # Single-email or multi: Go convert handles batch when emails is None / pending.
    # Explicit email list still goes through Python path for precise selection unless empty.
    if use_go and (emails is None or len(emails) == 0 or (len(emails) == 1 and not only_pending)):
        try:
            from grok_register.polyglot import go_inventory_worker_bin, inventory_convert
            ...
            if go_inventory_worker_bin() is not None:
                if progress_cb:
                    try:
                        progress_cb(
                            ...
                            fail=0,
                            skipped=0,
                            message="go inventory-worker convert…",
                        )
                    except Exception:
                        pass
                email_one = ""
                if emails and len(emails) == 1:
                    email_one = str(emails[0]).strip()
                go_out = inventory_convert(
                    ...
                    enroll=allow_enroll,
                    limit=limit,
                    workers=max(1, int(os.environ.get("CONVERT_WORKERS") or "4")),
                ...
                msg = (
                    f"转换完成（Go）：成功 {ok} · 失败 {fail} · 跳过 {skipped} · {elapsed}s"
                )
                if need_sso and fail and not allow_enroll:
                    msg += f"（{need_sso} 个仅有 SSO、无 OAuth，请点「仅转换待 OAuth」走协议 enroll）"
                return {
