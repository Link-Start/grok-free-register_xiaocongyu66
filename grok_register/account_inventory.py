...
def ensure_bundles(*, rebuild: bool = False) -> dict[str, str]:
    ...
    prefer_rust = (os.environ.get("INVENTORY_ENGINE") or "rust").strip().lower() != "python"
    if prefer_rust and (rebuild or not (root / "sub2api" / "accounts.sub2api.json").is_file()):
        try:
            from grok_register.polyglot import rust_inventory_worker_bin, rust_rebuild_bundles

            if rust_inventory_worker_bin() is not None:
                paths = rust_rebuild_bundles(root)
                out = dict(paths.get("paths") or paths)
                # Drop any merge bundle paths rust may still report
